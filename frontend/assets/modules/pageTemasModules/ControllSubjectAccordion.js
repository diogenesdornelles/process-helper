...
export default class ControllSubjectAccordion {
  ...
  static getItemsAccordion () {
    ...
    for (let i = 0; i < headers.length; i++) {
      if (!i) {
        ControllSubjectAccordion.accordionItems.push({
  ...
  static fireAccordion () {
    ...
    ControllSubjectAccordion.accordion = new Accordion(ControllSubjectAccordion.accordionItems, ControllSubjectAccordion.accordionOptions)
    const headers = document.querySelectorAll('.accordion-heading')
    headers.forEach(header => {
      header.addEventListener('click', (el) => {
        ControllSubjectAccordion.accordion.open(`${el.target.getAttribute('id')}`)
        textareaAutoExpand()
        setTimeout(() => {
  ...
  static start () {
    ControllSubjectAccordion.accordionItems = []
    ControllSubjectAccordion.getItemsAccordion()
    ControllSubjectAccordion.fireAccordion()
  }
}
